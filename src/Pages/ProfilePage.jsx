...
export const ProfilePage = () => {
  ...
  const handleAvatarChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.size > 2 * 1024 * 1024) {
        toast.error("Hey file size is too large", { position: "top-center" });
        return;
      }
      setAvatar(file);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (avatar) {
        const filexEx = avatar.name.split(".").pop();
        const fileName = `${user.id}-${Math.random()
          .toString(36)
          .substring(2)}`;
        const filePath = `avatars/${fileName}.${filexEx}`;

        const { error: uplaodError } = await supabase.storage
          .from("avatars")
          .upload(filePath, avatar);

        if (uplaodError) throw uplaodError;
